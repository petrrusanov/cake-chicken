...
# 0000000000000000 8d39c3e9d85d044c7f5f Commiter <commiter@site.com> 1529212777 -0500	commit (initial): rev 1234
e=${log_elems[3]}
if [[ ${e:0:1} == "<" ]]; then
    ts=$(date -d "@${log_elems[4]}" +'%Y%m%dT%H:%M:%S')
fi

# 0000000000000000 8d39c3e9d85d044c7f5f Commiter Name <commiter@site.com> 1529212777 -0500	commit (initial): rev 1234
e=${log_elems[4]}
if [[ ${e:0:1} == "<" ]]; then
